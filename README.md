...
本项目用于附件六股票期权接入测试：在 Windows 64 位实体机上验证交易前置连接、客户端认证、账户登录、资金查询，以及行情前置连接和登录；还提供受显式开关保护的“单次限价报单后自动撤单”基础功能测试。源码版本为 `0.2.1`，绑定个股期权 SDK `v3.7.5_CP_20251125`。公开仓库包含源码、头文件、LIB 和错误码文件；两只原厂运行时 DLL 需从用户持有的 SDK 压缩包复制到本地，再在 Windows 上编译生成 EXE。
...
AppID 中的 `v1.0.0` 是已申请的标识组成部分，不能因为样例源码版本是 `0.2.1` 就随意修改。投资者代码若与登录账号不同，应按中信提供的值修改 `investor_id`。程序只允许上述评测前置和 BrokerID，不支持将配置直接切到生产环境。
...
2. **交易策略**：执行可重复审计的 `single-shot-limit` 单次策略。参数通过命令行输入，策略只生成一次委托；委托进入排队状态后通过 `ReqOrderAction` 自动撤销剩余数量。它不根据行情作投资判断，不重试、不追价，也不会循环报单。
...
`--send-order` 存在真实成交风险：即使程序在排队后立即申请撤单，委托也可能先成交或部分成交。若全部成交，程序会把撤单验证记为失败并明确记录实际成交；它不会自动再发第二笔平仓单。此时应立即在柜台核对委托、成交和持仓。只有日志最终出现 `BASIC RESULT status=PASS ... order=PASS cancel=PASS`，才能说明本次实测同时验证了报单接受和程序撤单；报单被拒、超时或已经成交都不能冒充撤单通过。
...
`Req...` 返回 0 只说明 API 接受了请求，不等于柜台已接受登录。应继续看 `OnRsp...` 响应中的 `ErrorID`、请求编号和最后一条响应标志。资金查询收到正常结束但没有记录，会注明空结果；它不等于“资金为零”。各阶段和最终结果会同时写入屏幕和 `logs` 下的独立日志。v0.2.1 起运行日志只输出可打印 ASCII：柜台返回的非 ASCII 本地语言文本不会原样打印，程序改为输出英文状态枚举，并保留其中连续的数字错误线索（例如 `numeric_tokens=5009`）。API 内部流文件在 `flow` 下，交易与行情分开存放。
