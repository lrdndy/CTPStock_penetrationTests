...
本项目用于附件六股票期权接入测试：在 Windows 64 位实体机上验证交易前置连接、客户端认证、账户登录、资金查询，以及行情前置连接和登录；还提供受显式开关保护的单次限价报单测试，可选择撤单或等待成交，并可按报备表配置每日和每秒最大报单量风控。源码版本为 `0.4.0`，绑定个股期权 SDK `v3.7.5_CP_20251125`。公开仓库包含源码、头文件、LIB 和错误码文件；两只原厂运行时 DLL 需从用户持有的 SDK 压缩包复制到本地，再在 Windows 上编译生成 EXE。
...
AppID 中的 `v1.0.0` 是已申请的标识组成部分，不能因为样例源码版本是 `0.4.0` 就随意修改。投资者代码若与登录账号不同，应按中信提供的值修改 `investor_id`。程序只允许上述评测前置和 BrokerID，不支持将配置直接切到生产环境。
...
先在 `config\connection.local.ini` 的 `[risk]` 中把 `daily_max_order_count` 和 `per_second_max_order_count` 分别填为报备表上的正式整数，范围均为 `1..999999999`。程序不内置或猜测阈值；基础实发模式以及风控设置/触发自测要求两个值齐全。基础实发缺少任一阈值时，会在读取凭据和连接前拒绝启动；普通连接测试和基础 dry-run 不要求填入阈值。升级用户需要在原本的每日配置旁补上每秒配置。修改 INI 后不需重新编译。
...
该计数范围是“本机器、本项目目录内、当前 BrokerID 与 UserID”，不会聚合其他机器、其他软件或其他项目副本发出的报单。正式报备口径若要求这些来源合并计数，必须先调整实现。单次测试程序每次只发一笔，连续手动启动通常无法达到较高的每秒阈值；使用 `trigger-second` 检查边界提示，不要用批量发单循环制作截图，也不要清空计数绕过风控。
...
配置文件中的凭据优先于进程环境变量。需要改密码、认证码或任一风控阈值时，编辑 `config/connection.local.ini` 并重新运行即可；修改配置本身不需要重新编译。升级到本版本的源码后仍需先重新编译一次。v0.1.3 的行情登录修复和 v0.2.2 的中文回报显示已由用户在 Windows 实机验证；2026-09-15 用户日志也记录了一次 SSE 合约报单接受、进入排队后成功撤单。v0.4.0 的 Windows 构建、两项风控弹窗和等待成交结果必须以你拉取后的本机执行与现场证据为准。
...
当前基础功能覆盖单次限价报单、自动撤单和等待成交目标，并已实现可配置、持久计数和截图自测的每日及每秒最大报单量风控。有效合约行情订阅、委托/成交/持仓查询展示及异常价格检查仍需继续实现。两个正式数量阈值必须以报备表为准，不能自行填演示值作为正式验收值。
