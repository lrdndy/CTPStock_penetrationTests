...
本项目是为附件六股票期权接入测试准备的第一步：在 Windows 64 位实体机上验证交易前置连接、客户端认证、账户登录、资金查询，以及行情前置连接和登录。源码版本为 `0.1.3`，绑定个股期权 SDK `v3.7.5_CP_20251125`。公开仓库包含源码、头文件、LIB 和错误码文件；两只原厂运行时 DLL 需从用户持有的 SDK 压缩包复制到本地，再在 Windows 上编译生成 EXE。
...
AppID 中的 `v1.0.0` 是已申请的标识组成部分，不能因为样例源码版本是 `0.1.3` 就随意修改。投资者代码若与登录账号不同，应按中信提供的值修改 `investor_id`。第一阶段程序只允许上述评测前置和 BrokerID，不支持将配置直接切到生产环境。
...
2026-09-14 实机对照发现：向当前股票期权行情前置提交 `ReqUserLogin(..., 1)` 后，成功的 `OnRspUserLogin` 回调把请求编号返回为 `0`。v0.1.3 仅在 MD 登录及其错误回调阶段兼容该值，并在日志中同时记录实际回调编号；Trader 认证、登录和资金查询仍严格匹配原请求编号。不能把此兼容规则扩展到所有交易回调。
